...
// city name with country code to get long and lat coordinates
//   http://api.openweathermap.org/geo/1.0/direct?q=toronto,CA&appid=c8334cad6bb5355d5be9600b1a12f31c

// country code needs to use iso (ISO 3166: Alpha-2 Code) standard 



// call to get current-time weather data using long and lat coordinates
//       https://api.openweathermap.org/data/2.5/weather?units=metric&lat={lat}&lon={lon}&appid={API key}


http://api.openweathermap.org/data/2.5/forecast?units=metric&lat=43.70&lon=-79.41&appid=c8334cad6bb5355d5be9600b1a12f31c
...
function getTodaysWeather(lon, lat, cityName) {
    ...
    .then(function (data) {
        ...
        let weatherDescription = data.weather.description; // weather description for current time
        let actualTemp = data.main.temp; // actual temperature of current
        let feelsLikeTemp = data.main.feels_like; // feels-like temperature of current time
        let windSpeed = data.wind.speed; // wind speed of current time
        let humidity = data.main.humidity; // humidity of current time
    ...
    fetch(queryURL, {
        ...
        .then(function (response) {
            ...
            for (let i=0; i < 3; i++) {
                ...
                let weatherDescription = daysList[i].weather.description; // weather description for this hour
                let actualTemp = daysList[i].main.temp; // actual temperature of this hour
                let feelsLikeTemp = daysList[i].main.feels_like; // feels-like temperature of this hour
                let windSpeed = daysList[i].wind.speed; // wind speed of this hour
                let humidity = daysList[i].main.humidity; // humidity of this hour
            }
